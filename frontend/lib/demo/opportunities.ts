...
export const demoOpportunities: Opportunity[] = items.map(([title, region, category, oneLineSummary, marketScore, confidenceScore, momentum, whyNow], index) => ({
 id: `demo-${index + 1}`, title, region, category, oneLineSummary, marketScore, confidenceScore, momentum, whyNow, ...base,
 evidence: [{ query: title, excerpt: `我正在寻找能解决「${oneLineSummary}」的工具，现有方式太耗时间。`, platform: index % 2 ? "Zhihu" : "Manual demo", observedAt: "2026-09-02", region, sourceUrl: "https://example.com/demo-signal" }]
}));
